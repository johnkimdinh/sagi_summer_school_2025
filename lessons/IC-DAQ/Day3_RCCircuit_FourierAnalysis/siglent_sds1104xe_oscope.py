""" siglent_sds1104xe_oscope.py

This module provides a set of basic functions to communicate with the Siglent SDS1104xe Oscilloscope.
Sam Condon, 2025-07-16
"""
...
def capture_data(inst, channel):
    """
    Captures data from the most recent trigger 

    :param inst: pyvisa resource.
    :param channel: Integer channel to capture data from.
    ...
    sleep(0.1)
    raw_data = inst.query_binary_values(f'C{channel}:WF? DAT2', datatype = 'B')
    data = []
    for d in raw_data:
        if d > 127:
            data.append(d - 256)
